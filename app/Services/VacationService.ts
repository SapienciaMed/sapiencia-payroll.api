...
export interface IVacationService {
  ...
  updateVacation(
    vacationEdit: IEditVacation,
    trx: TransactionClientContract
  ): Promise<ApiResponse<IVacation>>;
  updateVacationDay(
    data: IVacationDay
  ): Promise<ApiResponse<IVacationDay | null>>;
  getVacationsByParams(
    params: IVacationSearchParams
  ): Promise<ApiResponse<IVacation | null>>;
  createManyVacation(
    vacations: IVacationDayValidator,
    trx: TransactionClientContract
  ): Promise<ApiResponse<IVacationDay[]>>;
  getVacationPaginate(
    filters: IVacationFilters
  ): Promise<ApiResponse<IPagingData<IVacation>>>;
  getVacationsPeriodsByEmployment(): Promise<ApiResponse<IVacationPeriods[]>>;
}
...
export default class VacationService implements IVacationService {
  ...
  async createManyVacation(
    ...
    const vacationEnjoyed =
      await this.vacationDaysRepository.createManyVacation(
  ...
  async getVacationPaginate(
    ...
    const vacations = await this.vacationRepository.getVacation(filters);
    return new ApiResponse(vacations, EResponseCodes.OK);
  }

  async getVacationsPeriodsByEmployment(): Promise<
    ApiResponse<IVacationPeriods[]>
  > {
    const res = await this.vacationRepository.getVacationsPeriodsByEmployment();

    const periods: IVacationPeriods[] = res
      .flatMap((vacation) =>
        (vacation.vacationDay ?? [])
          .filter((days) => days && !days.paid)
          .map((days) => ({
            periods: `${vacation.period} - ${days.dateFrom} - ${days.dateUntil}`,
            payroll: days.codForm ?? 0,
            employment: vacation.codEmployment,
          }))
      )
    ...
    return new ApiResponse(periods, EResponseCodes.OK);
  }
}
